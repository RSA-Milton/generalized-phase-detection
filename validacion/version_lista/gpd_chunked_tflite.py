...
def process_chunk(chunk_data, chunk_start_time, dt, net, sta, output_file):
    ...
    try:
        ...
        max_vals = np.max(np.abs(tr_win), axis=1, keepdims=True) + 1e-9  # shape (B,1,3)
        tr_win = tr_win / max_vals
        
        tt = tt[:min_windows]
        
        # Predicción
        # Ajustar (solo si el ultimo batch no llena batch_size)
        if tr_win.shape[0] != inp["shape"][0]:
            interpreter.resize_tensor_input(inp["index"], [tr_win.shape[0], 400, 3])
            interpreter.allocate_tensors()
            # Refrescar detalles en las MISMAS globales
            inp = interpreter.get_input_details()[0]
            out = interpreter.get_output_details()[0]

        # Ejecutar
        interpreter.set_tensor(inp["index"], tr_win.astype(np.float32))
        interpreter.invoke()
        ts = interpreter.get_tensor(out["index"])
...
def main():
    ...
    interpreter = tf.lite.Interpreter(model_path="./models/gpd_v2.tflite", num_threads=4)  
